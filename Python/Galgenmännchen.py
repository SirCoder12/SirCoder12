...
def richtigen_buchstaben_anzeigen(buchstabe,angezeigtes_wort,lösung):
	...
	for zahl in range (anzahl_der_richtigen_buchstaben):
		...
		if lösung.find(buchstabe) == 0 and zahl == 0:
...
def galgenmännchen_zeichnen(anzahl_der_fehler):
	#Zuerst geben wir Turtle die Höchstgeschwindigkeit, setzen ihre Rotation und machen sie unsichtbar
	speed(10)
	hideturtle()
	setheading(0)
...
def schweres_wort_erhalten():
	with open("galgenmännchen_schwere_wörter.txt") as text:
		liste_wörter=text.read().split()
...
def galgenmännchen_leicht():
   ...
   #Liste zum überprüfen, ob der input regelgemäß ist
	...
	while lösung_gefunden == False and spiel_verloren == False:
		...
		elif (buchstabe == "joker" or "Joker") and (anzahl_der_joker != 0):
...
def galgenmännchen_zwei_spieler(steller, spieler, löser):
	...
	print(f"\nBitte gebe dein Wort ein {spieler[steller]}. Bitte beachte dabei das es nur die 26 Buchstaben des Alphabetes enthält und {spieler[löser]} es nicht sieht:")
	lösung=input("").lower()
	...
	if spiel_verloren == True:
		print("\nDu hast verloren", spieler[löser])
...
def main():
	print("""Galgenmännchen, ein einfaches Buchstabenspiel.
Von Mihail Zabolotnic (michail.zabolotnic@gmail.com)""")
	print("\nHallo Spieler, Willkommen bei Galgenmännchen! Kennst du die Regeln?")
	regeln_input=input("(ja , nein):")

	if regeln_input == "nein":
		print("""\nJe nach Spielmodus überlegt sich entweder der Computer oder der gegnerische Spieler ein Wort, das nicht angezeigt wird.
Alle Buchstaben des ausgedachten Wortes werden nur durch Striche angezeigt. Der Spieler nennt nun in beliebiger Reihenfolge nacheinander einzelne Buchstaben des Alphabets.
\nDer Computer/gegnerische Spieler muss nun jeweils anzeigen, wie oft und an welcher Stelle des Lösungswortes der Buchstabe vorkommt. So ergibt sich nach und nach das gesuchte Wort.
Kommt ein genannter Buchstabe darin jedoch nicht vor, so beginnt der Computer damit, einen Galgen mit einem Gehängten zu zeichnen.
\nDies geschieht in mehreren Etappen und bei jeder Fehlfrage kommt ein Teilstrich dazu, so dass der Rätsellöser 10 Fehlversuche hat.
Hat er dann das Wort noch nicht herausgefunden, so hat er verloren und hängt symbolisch am Galgen.""")
	
	anzahl_spieler=input("\nWillst du alleine gegen den Computer oder gegen einen anderen Spieler spielen? (1,2):")

	while anzahl_spieler.isdigit() == False:
		anzahl_spieler=input("Dies ist kein valider Wert. (1,2):")
	
	anzahl_spieler=int(anzahl_spieler)
	
	if anzahl_spieler == 2:
		print("\nKennt ihr die Regeln für zwei Leute?")
		regeln_zwei_spieler_input=input("(ja , nein)")

		if regeln_zwei_spieler_input == "nein":
			print("""\nEiner von euch ist der Steller. Er stellt das gesuchte Wort und organiziert das Spiel. Spieler 1 ist immer zuerst der Steller.
Der andere ist der Löser. Er muss das Wort erraten. Man sucht sich aus wieviele Runden gespielt werden. 
Die Rollen werden nach jedem Spiel getauscht. Der Spieler mit den meisten Siegen ist der Gewinner.""")
		
		spieler_1=input("\nBitte gebe deinen Namen ein Spieler 1:")
		spieler_2=input("Bitte gebe deinen Namen ein Spieler 2:")
		spieler={"Spieler 1": spieler_1, "Spieler 2": spieler_2}
		anzahl_runden=input("\nWie viele Runden wollt ihr spielen?:")
		...
		for zahl in range(anzahl_runden):
			...
			if galgenmännchen_zwei_spieler(steller,spieler,löser) == True:
				resultate["Gewinne Spieler 1:"]+=1
				resultate["Verluste Spieler 2:"]+=1
			else:
				resultate["Verluste Spieler 1:"]+=1
				resultate["Gewinne Spieler 2:"]+=1
			print(resultate)
		...
		schwierigkeitsgrad_regeln=input("Kennst du die Schwierigkeitsgrade? (ja , nein):")
		
		if schwierigkeitsgrad_regeln == "nein":
			print("\nEs gibt drei Schwierigkeitsgrade. Bei leicht kriegst du einen Joker für einen Buchstaben von irgendeiner Position und normale Wörter")
		...
		while ende != "nein":
			...
			ende=input("Möchtest du noch eine Runde spielen? (ja , nein)")
main()